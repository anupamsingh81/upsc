...
Sometime back UPSC 2016 results were declared and amidst usual hype and hoopla, a blog post bty twitter handle yugaparivatan [claimed](http://yugaparivartan.com/2017/06/03/does-upsc-favours-muslims-in-ias-interview/), that higher interview marks were awarded to Muslims . Some people insinuated same was happening for female candidates and wondered if it was by design or random event. 

I saw the data analysed it and was sitting on it for some time, then decided to analyse the whole thing along with other interesting factoids such as yearly and surname wise variation.
...
2013,2014,2015,2016 UPSC list of candidates with categories were collected. 
2015 list was pdf only,hence was extracted back to text then csv
surnames were extracted with Regex
The code for extraction and csv can be found here and here
...
So, the fairly symmetrical curve becomes almost trimodal with three peaks, what could be the potential cause..
...
summary(g)
```
We see that **even when we exclude 2016 (as one-ogff random event), being a Muslim definitely provides upto 10 point advantage in interview process in top rankers** and is infact the most significant of predictive factors excluding systematic yearly variations.
...
```
We see that the best ranks are secured by brahmin ,bania communities event though absolute numbers may be varibale
